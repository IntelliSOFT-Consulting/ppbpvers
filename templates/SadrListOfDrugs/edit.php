...
<div class="row">
    <aside class="column">
        <div class="side-nav">
            ...
            <?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $sadrListOfDrug->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $sadrListOfDrug->id), 'class' => 'side-nav-item']
            ) ?>
            <?= $this->Html->link(__('List Sadr List Of Drugs'), ['action' => 'index'], ['class' => 'side-nav-item']) ?>
        </div>
    </aside>
    <div class="column-responsive column-80">
        <div class="sadrListOfDrugs form content">
            <?= $this->Form->create($sadrListOfDrug) ?>
            <fieldset>
                <legend><?= __('Edit Sadr List Of Drug') ?></legend>
                <?php
